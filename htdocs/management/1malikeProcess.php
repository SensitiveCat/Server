...
if($nick == $liked) {
  ...
  $sql = "SELECT * FROM maliketable WHERE commentid=$id";
  ...
  if( $row['nick'] == $nick) {
    ...
    $sql = "INSERT INTO maliketable(nick, liked, commentid) VALUES ('".$nick."','".$liked."','".$id."')";
    $result = mysqli_query($conn, $sql);
    $sql = "UPDATE macomment SET liked = liked + 1 WHERE id=$id";
    $result = mysqli_query($conn, $sql);
