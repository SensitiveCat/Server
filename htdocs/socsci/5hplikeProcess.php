...
if($nick == $liked) {
  ...
  $sql = "SELECT * FROM hpliketable WHERE commentid=$id";
  ...
  if( $row['nick'] == $nick) {
    ...
    $sql = "INSERT INTO hpliketable(nick, liked, commentid) VALUES ('".$nick."','".$liked."','".$id."')";
    $result = mysqli_query($conn, $sql);
    $sql = "UPDATE hpcomment SET liked = liked + 1 WHERE id=$id";
    $result = mysqli_query($conn, $sql);
