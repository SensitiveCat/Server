...
if($nick == $liked) {
  ...
  $sql = "SELECT * FROM euliketable WHERE commentid=$id";
  ...
  if( $row['nick'] == $nick) {
    ...
    $sql = "INSERT INTO euliketable(nick, liked, commentid) VALUES ('".$nick."','".$liked."','".$id."')";
    $result = mysqli_query($conn, $sql);
    $sql = "UPDATE eucomment SET liked = liked + 1 WHERE id=$id";
    $result = mysqli_query($conn, $sql);
