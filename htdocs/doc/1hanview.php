...
$id = $_GET['id'];
$sql = "S1hanT * FROM 1han WHERE id=$id";
   ...
   <p>
    ...
    <article class="contents">
      <div class="jumbotron">
          ...
          require('../config/config2.php');
          $conn = db_init($config2["host"], $config2["duser"], $config2["dpw"], $config2["dname"]);
          $sql = "S1hanT ROUND(AVG(score), 2) FROM 1hancomment WHERE proid=$id";
          $result = mysqli_query($conn, $sql);
          $row = mysqli_fetch_assoc($result);
        ...
        <div class="jumbotron">
          ...
          <?php
          $sql = 'S1hanT * FROM 1hancomment ORDER BY liked DESC, created DESC LIMIT 3';
          $result = mysqli_query($conn, $sql);
                ...
                if(!isset($_SESSION['is_login'])){
                  ?><input type="submit" name="name" value="Like : <?php echo $row['liked']?>" class="likebtn btn-primary" style="border-radius: 30px;border:0px solid black;"><?php
                    ...
                    if(isset($_SESSION['id'])){?>
                          <form class="" action="1hanlikeProcess.php" method="post">
                        ...
                        if($_SESSION['id'] == $row['nick']) {?>
                          <form class="" action="1hancommentdelete.php" method="post">
                          <input type="hidden" name="nick" value="<?php echo $_SESSION['id']?>">
                          <input type="hidden" name="liked" value="<?php echo $row['nick']?>">
                          <input type="hidden" name="id" value="<?php echo $row['id']?>">
        ...
        <div class="jumbotron">
          <h2 class="text-center">COMMENTS</h2>
            ...
            $sql = "S1hanT count(*) as cnt FROM 1hancomment WHERE proid=$id ORDER BY created DESC";
            $result = mysqli_query($conn, $sql);
            $row = mysqli_fetch_assoc($result);
            ...
            if($page != 1) {
              $paging .= '<td class="page page_start"><a class="text-center btn btn-default" href="./1hanview?id='.$id.'&page=1">처음</a></td>';
            }
            if($currentSection != 1) {
              $paging .='<td class="page page_prev"><a class="text-center btn btn-primary" href="./1hanview?id='.$id.'&page='.$prevPage.'">이전</a></td>';
            }

            for($i = $firstPage; $i <= $lastPage; $i++) {
              if($i == $page) {
                $paging .='<td class="page current"><a class="text-center btn btn-info" href="./1hanview?id='.$id.'page='.$i.'">'.$i.'</a></td>';
              }
              else {
                $paging .='<td class="page"><a class="text-center btn btn-primary" href="./1hanview?id='.$id.'&page='.$i.'">'.$i.'</a></td>';
              }
            }

            if($page != $allPage) {
              $paging .='<td class="page page_end"><a class="text-center btn btn-default" href="./1hanview?id='.$id.'&page='.$allPage.'">끝</a></td>';
            }
            $paging .='</tr></table></div>';
            ...
            $sql = "S1hanT * FROM 1hancomment WHERE proid=$id ORDER BY created $sqlLimit";
            $result = mysqli_query($conn, $sql);
          }
          else {
            $sql = "S1hanT * FROM 1hancomment WHERE proid=$id ORDER BY created";
            $result = mysqli_query($conn, $sql);
          }
            ...
            if($result->num_rows >= 1) {
              ...
              <tr>
                ...
                <tr><td class="like">
                  ...
                  //$sql = "S1hanT liked FROM comment WHERE nick='".$commentuser."'";
                  //$result = mysqli_query($conn, $sql);
                  //$row = mysqli_fetch_assoc($result);
                    ...
                    ?><input type="submit" name="name" value="Like : <?php echo $row['liked']?>" class="likebtn btn-primary" style="border-radius: 30px;border:0px solid black;"><?php
                      ...
                      if(isset($_SESSION['id'])){?>
                          ...
                          if($_SESSION['id'] == $row['nick']) {?>
                            ...
                            <input type="hidden" name="id" value="<?php echo $row['id']?>">
                            <input type="submit" name="name" value="댓글삭제" class="delbtn btn-danger" style="font-size:0.9em;border-radius: 30px;border:0px solid black;">
                          </form><form class="" action="1citycommentupdate.php?page=<?php echo $page?>&id=<?php echo $id ?>" method="post">
                                <input type="hidden" name="nick" value="<?php echo $_SESSION['id']?>">
                                <input type="hidden" name="liked" value="<?php echo $row['nick']?>">
                                <input type="hidden" name="id" value="<?php echo $row['id']?>">
        ...
        <div class="jumbotron">
          ...
          <?php
            if(!isset($_SESSION['is_login'])) {
              ...
              ?>
            <form class="" action="1hancommentprocess?id=<?php echo $id?>" method="post">
              <h1 class="text-left">강의 수준</h1>
              <s1hant name="level" class="form-control text-center" style="height: 50px;">
              ...
              <s1hant name="mood" class="form-control text-center" style="height: 50px;">
               <option class="text-center" value = "">선택</option>
               <option class="text-center" value = "0">0점: 평가할 가치가 없다</option>
              ...
              <s1hant name="qual" class="form-control text-center" style="height: 50px;">
               <option class="text-center" value = "">선택</option>
               <option class="text-center" value = "0">0점: 평가할 가치가 없다</option>
              ...
              <s1hant name="quan" class="form-control text-center" style="height: 50px;">
               <option class="text-center" value = "">선택</option>
              ...
              <s1hant name="reportlv" class="form-control text-center" style="height: 50px;">
               <option class="text-center" value = "">선택</option>
              ...
              <s1hant name="testlv" class="form-control text-center" style="height: 50px;">
               <option class="text-center" value = "">선택</option>
              ...
              <s1hant name="scorelv" class="form-control text-center" style="height: 50px;">
               <option class="text-center" value = "">선택</option>
              ...
              <s1hant name="book" class="form-control text-center" style="height: 50px;">
               <option class="text-center" value = "">선택</option>
              ...
              <s1hant name="insung" class="form-control text-center" style="height: 50px;">
               <option class="text-center" value = "">선택</option>
          ...
          <s1hant name="star" class="form-control text-center" style="height: 50px;">
           <option class="text-center" value = "">선택</option>
        ...
        <button onclick="location.href='../doc/1han'" class="btn btn-default btn-lg btn-block text-center">목록으로 돌아가기</button>
      <p id="footer">
